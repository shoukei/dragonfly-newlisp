...
;; This file is to be used for Dragonfly using sys-load instead of load. line 51
(module "crypto.lsp")
...
;; (set 'oauth-details (string (env "HOME") "/oauth-details.lisp"))
(set 'oauth-details "oauth-details.lisp")

(if (file? oauth-details)
    ;; (load oauth-details 'Oauth)
    (sys-load oauth-details 'Oauth)  ;Dragonfly has overridden load, use sys-load instead
    ;; Make a file named oauth-details.lisp in root directory with the 5 lines below to access twitter api
    (begin
...
(define (get-tweets base-url query-params)
   (let (oauth_nonce oauth_timestamp signing-key 
    ...
    (set 'request-query (make-request-query query-params))
    
    ; make a signature-base-string
    (set 'signature-base-string 
        (build-signature-base-string "GET" base-url (build-parameter-string query-params)))
...
;; (set 'query (list (list "count" "200") (list "screen_name" "newlisp")))

;; (set 'base-url "https://api.twitter.com/1.1/statuses/user_timeline.json")

;; (set 'results (Oauth:get-tweets base-url query))
